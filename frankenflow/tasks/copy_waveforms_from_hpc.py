...
class CopyWaveformsFromHPC(task.Task):
    ...
    def check_pre_staging(self):
        ...
        self.target_file = os.path.join(
            self.working_dir,
            os.path.basename(self.inputs["waveform_tar_file"]))

        # Make sure the tar file exists.
        files = self.sftp_client.listdir(
            os.path.dirname(self.inputs["waveform_tar_file"]))
        assert os.path.basename(self.inputs["waveform_tar_file"]) in files, \
            "Remote file '%s' does not exists." % (
    ...
    def run(self):
        self.sftp_client.get(self.inputs["waveform_tar_file"],
                             self.target_file)
